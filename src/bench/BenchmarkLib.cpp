...
void create_benmkng_file(string name) {
  ...
  if (!benmkng_file)
    cerr << "-E- Error: create_benmkng_file. Info: Imposible to create file"
         << endl;
...
void close_benmkng_file(void) { benmkng_file.close(); }

void write_benmkng_info(string info, int dat) {
  ios_base::fmtflags initialState = benmkng_file.setf(
      ios_base::fixed, ios_base::floatfield);  // saves initial state
...
void write_benmkng_newline() { benmkng_file << endl; }

void reset_peak_memory() { max_mem_usage = 0; }

void update_benmkng_memory(long long mem) {
  // we are interested on the peak value
  if (mem > max_mem_usage) max_mem_usage = mem;
}

long long get_mem_peak() { return max_mem_usage; }
...
double totalTime(void) {
  ...
  double t_usr = (double)ru.ru_utime.tv_sec +
                 (double)ru.ru_utime.tv_usec / 1000000.0;  // user time
  double t_sys = (double)ru.ru_stime.tv_sec +
                 (double)ru.ru_stime.tv_usec / 1000000.0;  // system time
  return t_usr + t_sys;
}
...
int memReadStats(int field) {
  ...
  if (in == NULL) return 0;
  int value;
  for (; field >= 0; field--) fscanf(in, "%d", &value);
  fclose(in);
  return value;
}
