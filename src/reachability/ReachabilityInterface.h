...
class ReachabilityInterface : public Manager {
 public:
   ...
   * space or not. The input state is provided as a vector of boolean values for
   * each state bit.
   *
  ...
  /**
   * This function computes the distance from the initial state to a specified
   * state. i.e., minimum cycles it takes the FSM to transition to the specified
   * state Example: Initial state s0 = 0; s1 = 0 Transition functions: s0' =
   * !s1; s1' = s0 FSM transitions: {False, False} {True, False} {True, True}
   *             {False, True}
   *             {False, False}
   *             ...
   ...
   * The transition function specifies the value of the state after the
   * transition. The transition functions can be composed of state variables and
   * inputs if present. Example: s0' = s0 XOR s1 Example: s1' = (s1 AND s0) OR
   * i0 The next state for s0 is defined as XOR of the current values of the
   * state bit s0 and s1 The next state for s1 is defined as the AND of the
   * current values of s1 and s0 ORed with input i0 An exception is thrown, if
   *  - The number of given transition functions does not match the number of
   * state bits
   *  - An unknown ID is provided
   *
   * @param transitionFunctions provide a transition function exactly for each
   * state bit
