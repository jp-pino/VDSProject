...
bool isEquivalent(const uniqueTable &BDD1, const uniqueTable &BDD2, int root1,
                  int root2) {
  ...
  if (root1 != root2 &&
      ((root1 == 0 || root1 == 1) || (root2 == 0 || root2 == 1)))
    return false;
  if (BDD1.at(root1).var_name != BDD2.at(root2).var_name) return false;
  return isEquivalent(BDD1, BDD2, BDD1.at(root1).low, BDD2.at(root2).low) and
         isEquivalent(BDD1, BDD2, BDD1.at(root1).high, BDD2.at(root2).high);
}
...
int main(int argc, char *argv[]) {
  ...
  while (!BDD1_if.eof()) {
    ...
    if (temp.find("Terminal Node: 1") != std::string::npos) {
      ...
      ss.clear();
      ss.str(temp);
      ss >> temp >> temp >> id >> temp >> temp >> temp >> top_var >> temp >>
          temp >> temp >> n.var_name >> temp >> n.low >> temp >> n.high;
